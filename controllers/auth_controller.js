...
const checkSubscriptionType = async (req, res, next) => {
    ...
    if (request_type === 'type') {
        ...
        // This could be a string, or actual regex (in which case it'll be an object)
        ...
        if ((request_value === 'SHOWS' || request_value === 'MOVIES') && filter) {
            ...
            if (typeof(filter) === "string" ? !request_value.includes(filter) : filter.test(request_value) != true) {
                res.status(401).json({
